...
{
    [CustomEditor(typeof(AbilityEventObj_ExecuteMethodTrack))]
    public class AbilityEventObj_ExecuteMethodTrackEditor : Editor
    {
        private SerializedProperty executionIntervalProperty;
        private SerializedProperty executeOnStartProperty;
        private SerializedProperty executeOnEndProperty;
        ...
        {
            executionIntervalProperty = serializedObject.FindProperty("executionInterval");
            executeOnStartProperty = serializedObject.FindProperty("executeOnStart");
            executeOnEndProperty = serializedObject.FindProperty("executeOnEnd");
        ...
        private void DrawTrackSettings()
        {
            ...
            EditorGUILayout.PropertyField(executionIntervalProperty);
            EditorGUILayout.PropertyField(executeOnStartProperty);
            EditorGUILayout.PropertyField(executeOnEndProperty);

            float interval = executionIntervalProperty.floatValue;
            if (interval <= 0f)
            {
                EditorGUILayout.HelpBox("Will execute every frame while event range is active.", MessageType.Info);
            }
            else
            {
                EditorGUILayout.HelpBox($"Will execute about {1f / interval:F2} times per second.", MessageType.Info);
            }
        }
        ...
        private void RefreshComponentCandidates(GameObject targetObject)
        {
            ...
            foreach (var comp in targetObject.GetComponents<MonoBehaviour>())
            {
        ...
        private Component ResolveSelectedComponent(GameObject selectedObject)
        {
            ...
            int currentMatch = 0;
            foreach (var comp in selectedObject.GetComponents<MonoBehaviour>())
            {
                if (comp == null) continue;
                Type t = comp.GetType();
                if (t.AssemblyQualifiedName == typeName || t.FullName == typeName || t.Name == typeName)
        ...
        private void SyncTrackNameWithMethod()
        {
            if (string.IsNullOrEmpty(methodNameProperty.stringValue)) return;

            string name = methodNameProperty.stringValue;
            int p = name.IndexOf('(');
